...
orders_joined as (
    select
        {{ dbt_utils.date_trunc('month','created_timestamp') }} as order_date,
        order_lines.sku,
        min({{ dbt_utils.date_trunc('month','created_timestamp') }}) over (partition by order_lines.sku) as min_order_date,
...
orders_calendar as (
    ...
    select
        order_lines_calendar.date_day as order_date,
    ...
    left join orders_joined on order_lines_calendar.date_day = orders_joined.order_date
        and order_lines_calendar.sku = orders_joined.sku
    where order_lines_calendar.date_day >= order_lines_calendar.min_order_date
...
select
    *,
    {{ dbt_utils.surrogate_key(['order_date', 'sku']) }} as id
from final
order by 2, 1
