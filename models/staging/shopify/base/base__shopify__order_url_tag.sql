    ...
    select
        {{
            fivetran_utils.fill_staging_columns(
                source_columns=adapter.get_columns_in_relation(ref('base__shopify__order_url_tag_tmp')),
                staging_columns=get_shopify_order_url_tag_columns()
            )
        }}
...
select
    *,
    {{ dbt_utils.surrogate_key(['order_id','key','source_relation']) }} as order_url_tag_globalid,
