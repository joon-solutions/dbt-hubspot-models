...
with base as (
    ...
    select * from {{ ref('base__shopify__transaction_tmp') }}
    ...
    select
        ...
        {{
            fivetran_utils.fill_staging_columns(
                source_columns=adapter.get_columns_in_relation(ref('base__shopify__transaction_tmp')),
                staging_columns=get_shopify_transaction_columns()
            )
        }}
...
final as (
    ...
    select 
        ...
        {{ dbt_date.convert_timezone(column='cast(authorization_expires_at as ' ~ dbt_utils.type_timestamp() ~ ')', target_tz=var('shopify_timezone', "UTC"), source_tz="UTC") }} as authorization_expires_at,
