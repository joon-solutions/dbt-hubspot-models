{{ config(enabled=var('marketo__activity_open_email', False }}
    ...
    select
        {{
            fivetran_utils.fill_staging_columns(
                source_columns=adapter.get_columns_in_relation(source('marketo', 'activity_open_email')),
                staging_columns=get_marketo_activity_open_email_columns()
            )
        }}
    from base
